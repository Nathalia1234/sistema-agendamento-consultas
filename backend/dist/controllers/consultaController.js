...
export const createConsulta = async (req, res) => {
    try {
        ...
        const result = await pool.query("INSERT INTO consultas (paciente_id, medico_id, data_consulta, descricao) VALUES ($1, $2, $3, $4) RETURNING *", [paciente_id, medico_id, data_consulta, descricao]);
...
export const updateConsulta = async (req, res) => {
    try {
        ...
        const result = await pool.query("UPDATE consultas SET descricao = $1, data_consulta = $2 WHERE id = $3 RETURNING *", [descricao, data_consulta, id]);
        res.status(200).json(result.rows[0]);
    }
    catch (error) {
        res.status(400).json({ error: error.message });
