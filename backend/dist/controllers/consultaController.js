...
export const createConsulta = async (req, res) => {
    try {
        ...
        const medico = await pool.query("SELECT id FROM medicos WHERE id = $1", [medico_id]);
        if (medico.rowCount === 0) {
            return res.status(404).json({ error: "Médico não encontrado." });
        }
        // Checar conflito de agenda
        const conflito = await pool.query("SELECT 1 FROM consultas WHERE medico_id = $1 AND data_consulta = $2", [medico_id, data_consulta]);
        ...
        // Criar consulta
        const result = await pool.query(`INSERT INTO consultas (paciente_id, medico_id, data_consulta, descricao)
...
export const getConsultas = async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM consultas ORDER BY data_consulta ASC");
...
export const updateConsulta = async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE consultas 
...
// ------------------------------------------------------------
export const cancelConsulta = async (req, res) => {
    try {
