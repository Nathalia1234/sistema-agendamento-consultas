...
export class ConsultaService {
    ...
    async criarConsulta(data) {
        ...
        const result = await pool.query("INSERT INTO consultas (paciente, medico, data_consulta, descricao) VALUES ($1, $2, $3, $4) RETURNING *", [paciente, medico, data_consulta, descricao]);
    ...
    async atualizarConsulta(id, data) {
        const { paciente, medico, data_consulta, descricao } = data;
        const result = await pool.query("UPDATE consultas SET paciente=$1, medico=$2, data_consulta=$3, descricao=$4 WHERE id=$5 RETURNING *", [paciente, medico, data_consulta, descricao, id]);
        return result.rows[0];
    }
    // Deleta uma consulta
    ...
    async cancelConsulta(consultaId) {
        ...
        const consultaExistente = await pool.query("SELECT id FROM consultas WHERE id = $1", [consultaId]);
